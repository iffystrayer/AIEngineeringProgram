 ...
 * Integrates with API client and progress tracking.
 */
export const LandingPage: React.FC<LandingPageProps> = ({
  ...
  const handleCreateSession = async (data: CreateSessionRequest) => {
    try {
      await createNewSession(data.user_id, data.project_name, data.description)
  ...
  const handleSelectSession = async (selectedSession: SessionResponse) => {
    try {
      setShowSessionModal(false)
      // Navigate to questionnaire with session ID
    ...
    <ErrorBoundary>
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              <p className="text-red-800">
                {sessionError?.message || progressError?.message}
              </p>
            </div>
          </div>
        )}
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          {/* Show Progress if Session Active */}
          {sessionId && progress && (
            <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="bg-white rounded-lg shadow-lg p-8 hover:shadow-xl transition-shadow">
              ...
              >
                {sessionLoading ? 'Loading...' : 'Start New Questionnaire'}
              </button>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-lg p-8 hover:shadow-xl transition-shadow">
              ...
              <button
                ...
                disabled={sessionLoading}
                className="w-full bg-green-600 text-white py-2 px-4 rounded-md hover:bg-green-700 disabled:bg-gray-400 transition-colors font-medium"
                data-testid="resume-button"
              >
                {sessionLoading ? 'Loading...' : 'View Sessions'}
              </button>
            </div>
          </div>
          ...
          <div className="fixed inset-0 z-40 bg-black bg-opacity-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
              <NewSessionForm
                ...
                onCancel={handleCloseNewSessionForm}
                isLoading={sessionLoading}
